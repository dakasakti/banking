@extends('layouts.dashboard') @section('content') @if(session('success')) @php
$type = session('type', 'green') @endphp
<div
    id="alert-border-3"
    class="flex p-4 mb-4 bg-{{ $type }}-100 border-t-4 border-{{
        $type
    }}-500 dark:bg-{{ $type }}-200"
    role="alert"
>
    <svg
        class="flex-shrink-0 w-5 h-5 text-{{ $type }}-700"
        fill="currentColor"
        viewBox="0 0 20 20"
    ...
    <button
        type="button"
        class="ml-auto -mx-1.5 -my-1.5 bg-{{ $type }}-100 dark:bg-{{
            $type
        }}-200 text-{{ $type }}-500 rounded-lg focus:ring-2 focus:ring-{{
            $type
        }}-400 p-1.5 hover:bg-green-200 dark:hover:bg-green-300 inline-flex h-8 w-8"
